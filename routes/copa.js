...
function calcularTabla(jugadores, partidos, clasificados) {
  const stats = {};
  jugadores.forEach(j => {
    stats[j.usuario_id] = { nick: j.nombre, elo: j.elo, pj: 0, pg: 0, pp: 0, sf: 0, sc: 0, pts: 0 };
  });
  partidos.forEach(p => {
    if (p.estado !== 'finalizado') return;
    const id1 = p.jugador1_id, id2 = p.jugador2_id;
...
router.get('/eventos', (req, res) => {
  const db = getDB();
  try {
    const torneos = db.prepare("SELECT * FROM torneos WHERE estado IN ('inscripcion', 'jugando') ORDER BY creado_en DESC").all();
    const activeTournaments = torneos.map(torneo => {
    ...
    res.render('torneos_uruguay', { activeTournaments });
  } finally {
    db.close();
  }
});
  ...
  try {
    ...
    const grupos = gruposRaw.map(g => {
      const participantes = db.prepare(`
        SELECT gp.participante_id, gp.usuario_id, gp.equipo_id, gp.nombre, gp.elo,
      ...
      const partidos = db.prepare(`
        SELECT p.*, u1.nombre AS j1_nombre, u2.nombre AS j2_nombre
        FROM partidos p
      ...
      return { ...g, participantes, partidos };
    });

    const fasesElim = ['octavos', 'cuartos', 'semis', 'final'];
    const eliminatorias = {};
    ...
    res.render('torneo', { torneo, inscriptos, grupos, eliminatorias });
  } finally {
    db.close();
  }
});
...
router.get('/copa_stats', (req, res) => {
  const db = getDB();
  try {
    const torneo = db.prepare('SELECT * FROM torneos WHERE slug = ?').get(COPA_SLUG);
    if (!torneo) {
...
router.get('/eventos', (req, res) => {
  ...
  try {
    ...
    `).all();
    res.render('eventos', { activeTournaments });
  } finally {
    db.close();
  }
});
...
router.get('/torneo/:slug', (req, res) => {
  ...
  try {
    ...
    const grupos = gruposRaw.map(g => {
      const participantes = db.prepare(`
        SELECT gp.usuario_id, gp.equipo_id, gp.elo, u.nombre AS usuario_nombre
        FROM grupo_participantes gp
        LEFT JOIN usuarios u ON gp.usuario_id = u.discordId
        WHERE gp.grupo_id = ?
        ORDER BY gp.elo DESC
